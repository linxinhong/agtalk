...
//!
//! 命令：
//!   discover  - 从当前 shell 环境发现 zellij endpoint，输出 JSON
//!   send      - 从 stdin 读取 notify payload，执行 zellij 提醒
//!   （无参数默认等价 send）
...
fn main() {
    ...
    match cmd {
        ...
        _ => {
            eprintln!("usage: agtalk-notify-zellij [discover | send [--dry-run]]");
            std::process::exit(1);
        }
    }
}
...
fn run_discover() {
    let output = match zellij::discover() {
        Ok((session, pane)) => protocol::DiscoverOutput::ready(session, pane),
...
fn run_send(dry_run: bool) {
    ...
    if let Err(e) = zellij::send(&input, dry_run) {
        eprintln!("{}", e);
        std::process::exit(1);
    }
}
