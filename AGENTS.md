...
- 弹窗窗口 480×480 不可调（早期参考 agtalk-office 的 420×320，为给正文/选项更多空间已两次调大）。
...
- **提交信息用中文**，简洁描述性。
- 一个 commit 一个逻辑变更。
- **及时提交**：完成一个可独立验证的逻辑变更后，应在验证通过后及时 `git add` + `git commit`，不要把多个不相关任务长期堆在 working tree。
- 提交前必须先看 `git status --short` 和 `git diff --stat`，只暂存本次任务相关文件；遇到他人/用户未提交改动，必须避开或明确说明，禁止顺手纳入自己的 commit。
- 若验证未跑完或存在已知失败，不要提交；除非用户明确要求保存阶段性成果，提交信息必须标注原因。
...
4. 提交前跑齐验证命令（第 6 节）。
5. 验证通过且变更可独立交付时，及时提交 git；一次提交只包含一个逻辑变更。
6. 文件超 500 行立即考虑拆分。
7. 遇到"特例"（某个域需要特殊处理）立即停下来——多半是架构没想清楚，先回 design.md 重新设计，而不是加 if 分支。
