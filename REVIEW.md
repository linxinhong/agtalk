...
> Review 是 Agent 自主行为，不是框架强制流程。Agent 基于自身判断决定是否发起 Review，以及找谁 Review。
...
```bash
agtalk send <reviewer_agent> "[TASK] Review 请求：..." --wait-done --timeout 120
```
...
```bash
# 确认继续
agtalk send <requester> "[REPLY] 方案确认，可以继续。理由：..."
agtalk done <message_id>

# 建议调整
agtalk send <requester> "[REPLY] 建议调整：<具体建议>。原因：..."
agtalk done <message_id>
```
